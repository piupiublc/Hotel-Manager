  ...
  return (
    <>
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-lg">Manage your account, business profile, and payout methods.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-1 gap-8 max-w-4xl">
            ...
            <form onSubmit={handlePersonalSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input required className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" value={personalForm.fullName} onChange={e => setPersonalForm({...personalForm, fullName: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-2">Phone Number</label>
                  <input className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" value={personalForm.phone} onChange={e => setPersonalForm({...personalForm, phone: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-2">Identity (CCCD)</label>
                  <input className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" value={personalForm.cccd} onChange={e => setPersonalForm({...personalForm, cccd: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-2">Avatar URL</label>
                  <input className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" value={personalForm.avatar} onChange={e => setPersonalForm({...personalForm, avatar: e.target.value})} />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold mb-2">Address</label>
                <textarea rows={2} className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3 shadow-inner" value={personalForm.address} onChange={e => setPersonalForm({...personalForm, address: e.target.value})} />
              </div>
            ...
            <form onSubmit={handleBusinessSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input required className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" value={businessForm.businessName} onChange={e => setBusinessForm({...businessForm, businessName: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-2">Business Type</label>
                  <select className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" value={businessForm.businessType} onChange={e => setBusinessForm({...businessForm, businessType: e.target.value})}>
              ...
              <div>
                ...
                <input className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" value={businessForm.address} onChange={e => setBusinessForm({...businessForm, address: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-bold mb-2">About Business</label>
                <textarea rows={3} className="w-full rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-3" placeholder="Briefly describe your services..." value={businessForm.description} onChange={e => setBusinessForm({...businessForm, description: e.target.value})} />
              </div>
              <div className="flex justify-end pt-4">
                <button type="submit" disabled={saving} className="bg-primary text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all">
        ...
        {activeTab === 'bank' && (
          <div className="space-y-6">
            ...
            {showBankForm && (
              <section className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-primary/30 p-6">
                ...
                <form onSubmit={handleBankSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input required placeholder="e.g. Vietcombank" className="w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-2" value={bankForm.bankName} onChange={e => setBankForm({...bankForm, bankName: e.target.value})} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold mb-1 uppercase tracking-wider text-slate-500">Account Number</label>
                      <input required placeholder="0123XXXXXXXX" className="w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-2" value={bankForm.accountNumber} onChange={e => setBankForm({...bankForm, accountNumber: e.target.value})} />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold mb-1 uppercase tracking-wider text-slate-500">Account Holder Name (NO SIGNS)</label>
                    <input required placeholder="NGUYEN VAN A" className="w-full rounded-lg border-slate-200 dark:border-slate-800 dark:bg-slate-950 px-4 py-2" value={bankForm.accountHolder} onChange={e => setBankForm({...bankForm, accountHolder: e.target.value.toUpperCase()})} />
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {bankAccounts.map((acc) => (
                <div key={acc.id} className={`p-6 rounded-xl border transition-all relative ${acc.isPrimary ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm'}`}>
                  {acc.isPrimary && (
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white leading-none mb-1">{acc.bankName}</p>
                      <p className="text-xs text-slate-500 font-medium">{acc.accountHolder}</p>
                    </div>
                  </div>
                  <p className="text-lg font-mono tracking-widest text-slate-700 dark:text-slate-300 mb-4">{acc.accountNumber}</p>
